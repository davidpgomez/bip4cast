...
var recordsNames = ['eeag', 'hdrs', 'ymrs', 'panss_pos', 'panss_neg', 'panss_gen'];
// auxiliary functions
...
exports.createPrescriptionsJSON = function(data, beginDate, endDate){
	if (data === null || data === undefined || data.length === 0)
		return undefined;
	var json = JSON.parse('[]');
	var pres_number = data.length;
	var index = 0;	// first available position in json array
	for (var d = beginDate; d<=endDate; d.setDate(d.getDate() + 1)){
		for(var i = 0; i < pres_number; i++){
			if(data[i].beginDate <= d && (data[i].endDate === null || data[i].endDate >= d)){
				// if the selected prescription is on range, we add it to json
				json[index] = {'date' : d.toISOString(), 'type' : data[i].type.toString(), 'count' : 1};
				index = index + 1;
			}
		}
	}
...
exports.createRecordsJSON = function(data){
    var json = JSON.parse('[]');
    var recordsNumber = data.length;
    for(var i = 0; i< recordsNumber; i++){
        var date = data[i].date;
        for(var j = 0; j< recordsNames.length; j++){
            json.push({'date' : date, 'type' : recordsNames[j], value : data[i][recordsNames[j]]});
        }
    }
    return json;
}
